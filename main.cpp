...
int main(int argc, char** argv)
{
    ...
    svmClassifier->setC(1.0);
    svmClassifier->setNu(0.0);
    svmClassifier->setP(0.0);
    svmClassifier->setTermCriteria(TermCriteria(CV_TERMCRIT_ITER, 1000, 0.01));
    ...
    for(int i = 0; i < plates.size(); i++) {
        ...
        cout << "Plate " << filename_withoutExt << " will be read by OCR." << endl;
