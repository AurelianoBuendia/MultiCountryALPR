...
void OCR::train(Mat TrainData, Mat classes, int nlayers) {
    int networkInputSize = TrainData.cols;
    std::vector<int> layerSizes = { networkInputSize, nlayers, numCharacters };
    ...
    ann->setTermCriteria(TC(max_iter, 0));
    ann->setTrainMethod(method, method_param);
    ...
    //Learn classifier
    //ORIGINAL: ann.train(TrainData, trainClasses, weights);
    ann->train(TrainData, cv::ml::ROW_SAMPLE, trainClasses);
    trained = true;
}
...
int OCR::classifyKnn(Mat f) {
    ...
    int response = (int)knnClassifier->findNearest(f, K, matResults);
    return response;
}
...
void OCR::trainKnn(Mat trainSamples, Mat trainClasses, int k) {
    ...
    knnClassifier->train(trainSamples, cv::ml::ROW_SAMPLE, trainClasses);
}
